...
import org.derpfest.app.IParallelSpaceManager;
...
public final class ParallelSpaceManagerService extends SystemService {
    ...
    private void broadcastChange() {
        ...
        Intent intent = new Intent(org.derpfest.content.Intent.ACTION_PARALLEL_SPACE_CHANGED);
        intent.addFlags(Intent.FLAG_RECEIVER_INCLUDE_BACKGROUND);
        getContext().sendBroadcastAsUser(intent, UserHandle.of(userId),
                org.derpfest.Manifest.permission.MANAGE_PARALLEL_SPACES);
    }
    ...
    public void onStart() {
        ...
        publishBinderService(DerpFestContextConstants.PARALLEL_SERVICE, mInterface);
    ...
    private void enforceCallingPermission(int uid) {
        if (uid == Process.SYSTEM_UID || uid == Process.SHELL_UID)
            return;
        if (!hasPermissionGranted(org.derpfest.Manifest.permission.MANAGE_PARALLEL_SPACES, uid))
